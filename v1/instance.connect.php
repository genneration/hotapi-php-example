...
if($arrayResponse["isValid"] == "true"){
	echo "<br><br><br>*********************************************************************************************************";
	echo "<br><br><b>AÇÕES SIMULADAS:</b> (verifica retorno da verificação ou tentativas de conexão)";
	...
	if($arrayResponse["result"]["state"] == "disconnected"){
		...
		//VERIFICA SE TEM ESSO NO CÓDIGO QR
		if($arrayResponse["result"]["qrCode"] == "IMAGE_ERROR"){
			echo "<br> - IMAGEM DO CÓDIGO QR: <b>HOUVE UM ERRO AO RECEBER A MENSAGEM, AGUARDE</b>";	
		}else{//if($arrayResponse["result"]["qrCode"] == "IMAGE_ERROR"){
			echo '<br> - IMAGEM DO CÓDIGO QR: <br><b><img src="data:image/png;base64,'.$arrayResponse["result"]["qrCode"].'"></b>';	
		}//else{//if($arrayResponse["result"]["qrCode"] == "IMAGE_ERROR"){
	}
